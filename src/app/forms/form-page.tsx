...
const COPY: Record<FormType, { description: string; example: string }> = {
  masu: { description: "把辞书形变成礼貌表达。", example: "食べます" },
  nai: { description: "把辞书形变成普通体否定。注意「ある→ない」。", example: "食べない" },
  ta: { description: "把辞书形变成过去／完成形式，规则与て形对应。", example: "食べた" },
  tai: { description: "把辞书形变成“想做……”的表达。", example: "食べたい" },
};
...
export function FormPage({ form }: { form: FormType }) {
  ...
  return (
    <main className="mx-auto flex min-h-dvh w-full max-w-2xl flex-col gap-8 px-6 py-10">
      ...
      <FormPractice verbs={verbs} form={form} example={copy.example} />
    </main>
  );
}
